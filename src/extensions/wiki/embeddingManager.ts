...
export class EmbeddingManager {
  ...
  /**
   * Purges cached embeddings that were generated by a model other than the
   * currently resolved one. Called after a successful re-index to bound the
   * cache size when the embedding model changes.
   *
   * @returns Number of stale entries removed (0 when no model is resolved)
   */
